...
Resource action children are rendered automatically through the resource route's outlet. Keep list pages focused on list content; do not add a wrapper solely to render `<Outlet />`, and do not replace the shared Create, Edit, or Show buttons with hard-coded `navigate()` calls. Set `outlet: "manual"` when a specialized layout renders `<Outlet />` or consumes `useOutlet()` itself, when a child should replace the list instead of overlaying it, or when the child must stay inside a parent provider or boundary. If an action button has no target, fix the route definition by ensuring the resource route name matches the API resource and the corresponding child declares `resourceAction`.
